        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 py-12 lg:py-16 relative">
            ...
            <div class="hidden lg:block absolute top-0 bottom-0 right-full pr-6 w-16 pointer-events-none">
                <div class="sticky top-32 flex flex-col items-center gap-4 py-5 px-3 rounded-2xl
                    ...
                    <div x-data="moderation(<?php the_ID(); ?>)">
                        <button @click="report()"
                            class="group w-10 h-10 flex items-center justify-center rounded-xl text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-200"
                            :class="reported ? 'text-red-500 bg-red-50 dark:bg-red-900/20' : ''"
        ...
        share() {
            ...
            if (navigator.share) {
                ...
                setTimeout(() => this.copied = false, 2000);
                    ...
                    <!-- Report -->
                    <button x-data="moderation(<?php the_ID(); ?>)" @click="report()"
                        class="w-9 h-9 flex items-center justify-center rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-200"
                        :class="reported ? 'text-red-500' : ''"
                        title="Signaler">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
                                d="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9" />
